...
async def get_user_from_db(user_id: str) -> UserValue | None:
    try:
        # get serialized data
        entry: bytes = await db.async_get(user_id)
...
async def get_user_from_db_for_update(user_id: str) -> UserValue:
    ...
    try:
        entry: bytes = await db.async_get_for_update(user_id)
    except RedisError:
...
async def handle_checkout_payment(msg):
    ...
    try:
        await db.async_set(req.user_id, msgpack.encode(user_entry))
    except RedisError as e:
...
async def handle_2pc_commit(msg):
    ...
    raw = await db.async_get_for_update(f"pending:{req.txn_id}")
    if raw:
        ...
        await db.async_set(pending["user_id"], msgpack.encode(user_entry))
        await db.async_delete(f"pending:{req.txn_id}")
